    ...
    var body: some View {
        ZStack {
            ...
            HomeView(showProfile: $showProfile)
                ...
                .offset(y: showProfile && viewState.height >= 0 ? viewState.height - 500 : viewState.height)
                .rotation3DEffect(
                    Angle(degrees: (showProfile && (viewState.height < 0) && (viewState.height > -500)) ? Double(viewState.height) / 5 - 10 : 0), axis: (x: 10, y: 0, z: 0), anchor: .top)
...
let screen = UIScreen.main.bounds                       // 全局常量，获取当前设备屏幕尺寸
